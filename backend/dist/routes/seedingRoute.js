...
const addToWatchlist = async (username, movieId) => {
    try {
        let user = await mongooseUsers_1.User.findOne({ username: username });
        if (!user) {
            logger_1.default.error("User not found. Creating user...");
            user = new mongooseUsers_1.User({
            ...
            await user.save();
            logger_1.default.debug("User created successfully:", user);
        }
        ...
        if (!user.watch_list.includes(movieId)) {
            ...
            await user.save();
            logger_1.default.debug(`Movie _id: ${movieId} added to ${username}'s watchlist.`);
        }
        else {
            logger_1.default.debug(`Movie _id: ${movieId} is already in ${username}'s watchlist.`);
        }
    }
    catch (error) {
        logger_1.default.error('Error adding movie to watchlist:', error);
    }
};
async function seedDatabase() {
    ...
    try {
        for (const movie of movies) {
            const existingMovie = await mongooseMovies_1.default.findOne({ id: movie.id.toString() });
            if (!existingMovie) {
                const newMovie = new mongooseMovies_1.default(movie);
                await newMovie.save();
                logger_1.default.debug(`Movie with _id: ${movie.id} added to the database.`);
                // Assuming addToWatchlist expects an ObjectId for the movie ID
                const movieObjectId = new mongoose_1.default.Types.ObjectId(movie._id);
                await addToWatchlist("testuser", movieObjectId);
                logger_1.default.debug(`Movie with _id: ${movie.id} added to testuser's watchlist.`);
            }
            else {
                // If the movie exists, convert ID to ObjectId before adding to the watchlist
                const movieObjectId = new mongoose_1.default.Types.ObjectId(movie._id);
                await addToWatchlist("testuser", movieObjectId);
                logger_1.default.debug(`Movie with _id: ${movie.id} already exists. Skipping.`);
            }
        }
        logger_1.default.debug('Database seeding completed.');
    }
    catch (error) {
...
//# sourceMappingURL=seedingRoute.js.map