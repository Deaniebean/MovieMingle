...
const InputFieldsMovie = () => {
    ...
    const queryParams = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post('http://localhost:8082/discover/movies', {
                genre: genreParams,
                years: yearsParams,
    ...
    return (
        <div>
            ...
            <label> 
                Genre
                <select name="genre" value={genre.map(String)} multiple onChange={(e) => setGenre(Array.from(e.target.selectedOptions, option => parseInt(option.value)))}>
            ...
            <label> 
                Round length
                <select name="rounds" value={rounds}onChange={(e) => setRounds(e.target.value)}>
